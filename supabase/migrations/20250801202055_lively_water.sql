...
ADD COLUMN IF NOT EXISTS clover integer DEFAULT 0;
...
BEGIN
  ...
  FROM friendships
  WHERE (requester_id = requester_profile_id AND addressee_id = target_profile_id)
     OR (requester_id = target_profile_id AND addressee_id = requester_profile_id);
  ...
  VALUES (requester_profile_id, target_profile_id, 'pending');
...
BEGIN
  ...
  AND addressee_id = addressee_profile_id
  AND status = 'pending';
  ...
  RETURN json_build_object(
    'success', true,
    'message', CASE 
      WHEN response = 'accepted' THEN 'Friend request accepted'
...
BEGIN
  ...
  SELECT EXISTS(
    ...
    WHERE ((requester_id = sender_profile_id AND addressee_id = receiver_profile_id) OR
           (requester_id = receiver_profile_id AND addressee_id = sender_profile_id))
    AND status = 'accepted'
  ...
  SELECT EXISTS(
    ...
    WHERE sender_id = sender_profile_id
    AND receiver_id = receiver_profile_id
  ...
  SELECT EXISTS(
    SELECT 1 FROM clover_transactions
    WHERE receiver_id = receiver_profile_id
    AND transaction_date = CURRENT_DATE
  ...
  -- Check if sender has enough clover (optional, if you want to require clover to send)
  SELECT clover INTO sender_clover
  FROM profiles
  ...
  VALUES (sender_profile_id, receiver_profile_id, 1, CURRENT_DATE);
  
  -- Update receiver's clover balance
  UPDATE profiles
  SET clover = clover + 1
  WHERE id = receiver_profile_id;
...
CREATE OR REPLACE FUNCTION get_friend_list(player_profile_id uuid)
RETURNS TABLE (
  friend_id uuid,
...
BEGIN
  ...
  SELECT 
    CASE 
      WHEN f.requester_id = player_profile_id THEN f.addressee_id
      ELSE f.requester_id
    END as friend_id,
    ...
    NOT EXISTS(
      ...
      WHERE ct.sender_id = player_profile_id
  ...
  JOIN profiles p ON p.id = CASE 
    ...
    ELSE f.requester_id
  END
  WHERE (f.requester_id = player_profile_id OR f.addressee_id = player_profile_id)
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(
      (SELECT f.status 
       FROM friendships f 
       WHERE (f.requester_id = searcher_profile_id AND f.addressee_id = p.id) OR
             (f.requester_id = p.id AND f.addressee_id = searcher_profile_id)
       LIMIT 1),
      'none'
    ) as friendship_status
  FROM profiles p
  WHERE p.id != searcher_profile_id
  AND (p.banned_until IS NULL OR p.banned_until < now())
  AND (
