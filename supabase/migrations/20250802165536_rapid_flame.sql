...
DROP TABLE IF EXISTS clover_transactions CASCADE;
...
CREATE TABLE clover_transactions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX clover_transactions_sender_id_idx ON clover_transactions(sender_id);
CREATE INDEX clover_transactions_receiver_id_idx ON clover_transactions(receiver_id);
CREATE INDEX clover_transactions_date_idx ON clover_transactions(transaction_date);
CREATE INDEX profiles_clover_idx ON profiles(clover);

-- Update existing profiles to have 0 clover if null
UPDATE profiles SET clover = 0 WHERE clover IS NULL;