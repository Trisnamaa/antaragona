/*
  # Add Online Status Tracking for Friends
  ...
  2. Functions
    ...
    - `get_online_friends()`: Get friends with their online status
    - `mark_user_online()`: Mark user as online
...
CREATE OR REPLACE FUNCTION is_user_online(p_profile_id uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  RETURN COALESCE(user_online, false);
END;
$$;
