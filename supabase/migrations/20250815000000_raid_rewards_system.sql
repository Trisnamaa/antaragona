-- Migration: Updated Raid Rewards System (10 Cards Pool)
-- Description: Generates 10 cards pool for each raid completion, players select 2 cards

-- 1. Drop existing raid_rewards table if exists (to avoid conflicts)
DROP TABLE IF EXISTS raid_rewards CASCADE;

-- 2. Create raid_card_pool table to store the 10 cards generated per raid room
...
-- 3. Create raid_player_selections table to track each player's selected cards
...
-- 4. Add raid titles to existing titles table (if not exists)
INSERT INTO titles (name, description, color, rarity, unlock_condition, is_active) VALUES
('Eclipse Master', 'Master of Eclipse Protocol - Complete 50 raids', 'gold', 'legendary', 'Complete 50 Eclipse Protocol raids', true),
('Neon Master', 'Master of Neon Warden - Complete 50 raids', 'gold', 'legendary', 'Complete 50 Neon Warden raids', true),
('Crimson Master', 'Master of Crimson Exodus - Complete 50 raids', 'gold', 'legendary', 'Complete 50 Crimson Exodus raids', true)
ON CONFLICT (name) DO NOTHING;

-- 5. Drop existing raid_progress table if exists (to avoid conflicts)
DROP TABLE IF EXISTS raid_progress CASCADE;

-- 6. Create raid_progress table (simplified - only for tracking completion)
...
CREATE INDEX IF NOT EXISTS idx_raid_progress_profile_map ON raid_progress(profile_id, map_id);

-- 8. Function to generate 10 cards pool for a raid room
...
-- 9. Function to claim selected cards and update player profile
...
-- 10. Function to get raid card pool for a room
...
-- 11. Function to get player's raid progress (integrated with existing title system)
CREATE OR REPLACE FUNCTION get_raid_progress(
    p_profile_id UUID
) RETURNS JSON AS $$
DECLARE
    v_progress JSON;
BEGIN
    SELECT json_agg(
        json_build_object(
            'map_id', rp.map_id,
            'map_name', rm.name,
            'completion_count', rp.completion_count,
            'last_completed_at', rp.last_completed_at,
            'master_title', COALESCE(
                (SELECT pt.is_equipped FROM player_titles pt 
                 JOIN titles t ON pt.title_id = t.id 
                 WHERE pt.profile_id = p_profile_id 
                 AND t.name LIKE rm.name || ' Master%' 
                 LIMIT 1), false
            )
        )
    ) INTO v_progress
    FROM raid_progress rp
    JOIN raid_maps rm ON rp.map_id = rm.id
    WHERE rp.profile_id = p_profile_id;
    
    RETURN COALESCE(v_progress, '[]'::JSON);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
GRANT SELECT, INSERT, UPDATE ON raid_progress TO authenticated;
...
GRANT EXECUTE ON FUNCTION get_raid_progress TO authenticated;
...
-- 14. RLS Policies
-- raid_card_pool: All authenticated users can view cards in their raid rooms
...
-- raid_player_selections: Users can only see their own selections
...
-- raid_progress: Users can only see their own progress
CREATE POLICY "Users can view own raid progress" ON raid_progress
    FOR SELECT USING (profile_id IN (
        SELECT id FROM profiles WHERE user_id = auth.uid()
    ));

CREATE POLICY "Users can update own raid progress" ON raid_progress
    FOR UPDATE USING (profile_id IN (
        SELECT id FROM profiles WHERE user_id = auth.uid()
    ));

CREATE POLICY "Users can insert own raid progress" ON raid_progress
    FOR INSERT WITH CHECK (profile_id IN (
        SELECT id FROM profiles WHERE user_id = auth.uid()
    ));